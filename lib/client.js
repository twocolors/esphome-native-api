...
class EsphomeNativeApiClient extends EventEmitter {
    constructor({
        ...
        this.connection.on('authorized', async () => {
            ...
            try {
                ...
                this.emit('error', e);
                if (this.connection.connected) this.connection.socket.end();
            }
        });
    ...
    disconnect() {
        if (initializeSubscribeBLEAdvertisements) this.connection.unsubscribeBluetoothAdvertisementService();
