...
class EsphomeNativeApiConnection extends EventEmitter {
    constructor({
        ...
        this.frameHelper.on('connect', async () => {
            ...
            try {
                ...
                const deviceInfo = await this.deviceInfoService();

                await this.authenticate(deviceInfo);
    ...
    parseEsphomeVersion(esphomeVersion) {
        if (typeof esphomeVersion !== 'string') return null;
        const match = esphomeVersion.match(/^(\d{4})\.(\d+)/);
    ...
    shouldUseLegacyAuthentication(deviceInfo) {
        const version = this.parseEsphomeVersion(deviceInfo?.esphomeVersion);
        if (!version) return false;
        return version.year < 2025 || (version.year === 2025 && version.month < 8);
    }
    async authenticate(deviceInfo) {
        const hasPassword = typeof this.password !== 'undefined' && this.password !== '';
        if (hasPassword) {
        ...
        if (this.shouldUseLegacyAuthentication(deviceInfo)) {
            const { invalidPassword } = await this.connectService(this.password);
            if (invalidPassword === true) throw new Error(`Invalid password`);
        }
    }
